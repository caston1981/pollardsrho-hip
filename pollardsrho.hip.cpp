...
void init_secp256k1() {
    ...
    #ifdef __HIPCC__
        hipMalloc(&preCompG, sizeof(ECPointJacobian) * (1ULL << windowSize));
        hipMalloc(&preCompGphi, sizeof(ECPointJacobian) * (1ULL << windowSize));
        hipMalloc(&jacNorm, sizeof(ECPointJacobian) * windowSize);
        hipMalloc(&jacEndo, sizeof(ECPointJacobian) * windowSize);
   #else
    ...
    G.infinity = 0;
        ...
        hipError_t err = hipMalloc(&dg, sizeof(ECPointJacobian));
        if (err != hipSuccess) {
            std::cerr << "hipMalloc failed: " << hipGetErrorString(err) << std::endl;
            exit(1);
        }

        hipMemcpy(dg, &G, sizeof(ECPointJacobian), hipMemcpyHostToDevice);
        pointDoubleJacobian(dg, dg);
        hipDeviceSynchronize();
        hipMemcpy(&H, dg, sizeof(ECPointJacobian), hipMemcpyDeviceToHost);
        hipFree(dg);
        hipFree(jacNorm);
        hipFree(jacEndo);
    #else
...
struct Buffers {
    ...
    Buffers() {
        #ifdef __HIPCC__
            hipMalloc(&d_R, sizeof(ECPointJacobian));
            hipMalloc(&d_G, sizeof(ECPointJacobian));
            hipMalloc(&d_H, sizeof(ECPointJacobian));
            hipMalloc(&d_k, sizeof(uint64_t) * 4);
            hipMemcpy(d_G, &G, sizeof(ECPointJacobian), hipMemcpyHostToDevice);
            hipMemcpy(d_H, &H, sizeof(ECPointJacobian), hipMemcpyHostToDevice);
        #endif
    }
...
uint256_t f(ECPointJacobian& R, uint256_t k, int key_range, Buffers& buffers) {
    ...
    #ifdef __HIPCC__
        hipMemcpy(buffers.d_R, &R, sizeof(ECPointJacobian), hipMemcpyHostToDevice);
    #endif
    ...
    #ifdef __HIPCC__
        hipMemcpy(&R, buffers.d_R, sizeof(ECPointJacobian), hipMemcpyDeviceToHost);
    #endif
...
uint256_t prho(std::string target_pubkey_hex, int key_range, int hares, bool test_mode) {
    ...
    for (int i = 0; i < hares; ++i) {
        hare_states[i].buffers = new Buffers();
            ...
            hipError_t err = hipMalloc(&d_point, sizeof(ECPointJacobian));
            if (err != hipSuccess) {
                std::cerr << "hipMalloc failed: " << hipGetErrorString(err) << std::endl;
                exit(1);
            }
            ...
            hipMemcpy(&hare_states[i].R, d_point, sizeof(ECPointJacobian), hipMemcpyDeviceToHost);
            hipFree(d_point);
        #else
            ECPointJacobian tmp;
    ...
    try {
        while (search_in_progress.load()) {
            ...
            for (int i = 0; i < hares; ++i) {
                ...
                #ifdef __HIPCC__
                    hipMemcpy(hare.buffers->d_k, k1_array + (4 - num_limbs), sizeof(uint64_t) * num_limbs, hipMemcpyHostToDevice);
                    scalarMultJacobian(hare.buffers->d_R, hare.buffers->d_k, key_range, windowSize);
                    if (should_sync.load()) hipDeviceSynchronize();
                    hipMemcpy(&pub1_jac, hare.buffers->d_R, sizeof(ECPointJacobian), hipMemcpyDeviceToHost);
                    hipMemcpy(hare.buffers->d_k, k2_array + (4 - num_limbs), sizeof(uint64_t) * num_limbs, hipMemcpyHostToDevice);
                    scalarMultJacobian(hare.buffers->d_R, hare.buffers->d_k, key_range, windowSize);
                    if (should_sync.load()) hipDeviceSynchronize();
                    hipMemcpy(&pub2_jac, hare.buffers->d_R, sizeof(ECPointJacobian), hipMemcpyDeviceToHost);
                #else
                ...
                if (!xfilled) {
                    ...
                    unsigned char compressed1[33], compressed2[33];
                        ...
                        hipMalloc(&d_compressed, 33);
                        hipMalloc(&d_pub_comp, sizeof(ECPoint));
                        hipMemcpy(d_pub_comp, &pub1, sizeof(ECPoint), hipMemcpyHostToDevice);
                        getCompressedPublicKey(d_compressed, d_pub_comp);
                        if (should_sync.load()) hipDeviceSynchronize();
                        hipMemcpy(compressed1, d_compressed, 33, hipMemcpyDeviceToHost);
                        hipMemcpy(d_pub_comp, &pub2, sizeof(ECPoint), hipMemcpyHostToDevice);
                        getCompressedPublicKey(d_compressed, d_pub_comp);
                        if (should_sync.load()) hipDeviceSynchronize();
                        hipMemcpy(compressed2, d_compressed, 33, hipMemcpyDeviceToHost);
                    ...
                    if (!test_mode) {
                        ...
                        if (x_equal && compare_uint256(hare.k1, hare.k2) != 0) {
                            ...
                            #ifdef __HIPCC__
                                hipMemcpy(hare.buffers->d_k, d_array + (4 - num_limbs), sizeof(uint64_t) * num_limbs, hipMemcpyHostToDevice);
                                scalarMultJacobian(hare.buffers->d_R, hare.buffers->d_k, key_range, windowSize);
                                hipDeviceSynchronize();
                                hipMemcpy(&test_point_jac, hare.buffers->d_R, sizeof(ECPointJacobian), hipMemcpyDeviceToHost);
                            #else
                            ...
                            if (test_point_jac.infinity == 1) {
                                ...
                                #ifdef __HIPCC__
                                    hipMemcpy(hare.buffers->d_k, found_key_array + (4 - num_limbs), sizeof(uint64_t) * num_limbs, hipMemcpyHostToDevice);
                                    scalarMultJacobian(hare.buffers->d_R, hare.buffers->d_k, key_range, windowSize);
                                    hipDeviceSynchronize();
                                    hipMemcpy(&verify_point_jac, hare.buffers->d_R, sizeof(ECPointJacobian), hipMemcpyDeviceToHost);
                                #else
                                ...
                                unsigned char compressed_verify[33];
                                    ...
                                    hipMalloc(&d_compressed_verify, 33);
                                    hipMalloc(&d_verify_comp, sizeof(ECPoint));
                                    hipMemcpy(d_verify_comp, &verify_point, sizeof(ECPoint), hipMemcpyHostToDevice);
                                    getCompressedPublicKey(d_compressed_verify, d_verify_comp);
                                    hipDeviceSynchronize();
                                    hipMemcpy(compressed_verify, d_compressed_verify, 33, hipMemcpyDeviceToHost);
                                    hipFree(d_compressed_verify);
                                    hipFree(d_verify_comp);
                                #else
